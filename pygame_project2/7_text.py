...
#즉 fps가 다르디고 속도가 다르면 안됨. 해결책은 포지션에 dt만큼 곱해줌 tick()안의 숫자를 넣으면 1000/숫자 값이 int형으로 나옴
    ...
    timer = game_font.render(str(int(total_time - elapsed_time)), True, (255, 255, 255))    #render함수(출력할 글자, True, 글자색상) 정수형으로 바꾸기 위해서 int로 싸줌. 그리고 render함수 안에 문자로 인식되게끔 str으로 감싸줌

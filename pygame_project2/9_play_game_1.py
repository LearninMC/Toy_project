...
enemy_x_pos = randrange(0, 411)     #X좌표는 랜덤(가로)
enemy_y_pos = 0                     #Y좌표는 맨 위에서 시작
...
# 2. 이벤트 처리 (키보드, 마우스 등)
    for event in pygame.event.get():     #사용자가 마우스를 움직인다던지, 키보드를 입력한다던지 하는 이벤트를 계속해서 받겠다는 뜻. 키보드르 누르면 무기를 발사한다던지 등등
        ...
        if event.type == pygame.KEYDOWN:    #키가 눌러졌는지 확인
            ...
            elif event.key == pygame.K_UP:      #윗키 눌러도 움직임 없음
                to_y = 0                    
            elif event.key == pygame.K_DOWN:    #아랫키 눌러도 움직임 없음
                to_y = 0
...
# 3. 게임 캐리터 위치 정의    
    ...
    enemy_y_pos += to_enemy_y * dt  #enemy는 계속해서 밑으로 내려가는 입력을 해줌
    #가로경계값 처리
    ...
    if character_y_pos < 0:
        ...
        character_y_pos = screen_height-character_height
    #만약 enemy가 땅에 닿으면 위로 올려줌
    if enemy_y_pos > (screen_height):
        print(enemy_y_pos)
        enemy_x_pos = randrange(0, 411)
        enemy_y_pos = 0 
#########################################################################################################################################
# 4. 충돌 처리
